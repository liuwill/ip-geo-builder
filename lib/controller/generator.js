...
exports.testJson = (req, res) => {
  const jsonData = { status: 'json', keys: Object.keys(global.sourceFiles) }
  jsonData['BASE_RESOURCE_PATH'] = process.env.BASE_RESOURCE_PATH
  res.json(jsonData)
}
...
exports.uploadHandler = (req, res) => {
  ...
  if (!global || !global.sourceFiles || !global.sourceFiles['CHINA_CITIES_PATH'] || !global.sourceFiles['ISO3166_PATH']) {
    res.json({ status: false, msg: '无法找到预加载的系统文件，请检查环境，联系开发者' })
    return
  }
